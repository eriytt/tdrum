...
MidiMessage::MidiMessage(uchar aCommand, uchar aP1, uchar aP2, int aTime) {
  time = aTime;
  command() = aCommand;
  p1() = aP1;
  p2() = aP2;
...
MidiMessage::MidiMessage (uchar *buf, size_t size, int aTime) :
  MidiMessage (buf[0], size > 1 ? buf[1] : 0,
  	       size > 2 ? buf[2] : 0,
  	       aTime)
{}
...
bool MidiMessage::is_note_on(void) {
...
bool MidiMessage::is_note_off(void) {
  if ((p0() & 0xf0) == 0x80)
    return true;
...
//    out << "(" << dec << aMessage.time << ")\t" 
//        << hex << (int)aMessage.getP0() << ": ";
//    for (int i=1; i<=aMessage.getArgCount(); i++) {
//       out << dec << (int)aMessage[i] << ' ';
//    }
